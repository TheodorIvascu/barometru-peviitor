...
// The whole analysis runs once a day, so the budget is a day's work, not a
// trickle: the COR residue alone is 300 ambiguous titles plus 300 the register
// has no wording for, eight to a call. The cap exists to stop a runaway loop,
// not to ration a scheduled job.
const DAILY_CALLS = Number(pick("LLM_DAILY_CALLS") || 400);
...
/**
 * Guard against a corrupt generation.
 *
 * A bulletin came back once with a Han character wedged mid-word - "ceea ce浪ipica"
 * - which is a decoding accident somewhere upstream, not a choice. It reached the
 * dashboard because nothing looked at the text before caching it. Romanian prose
 * uses Latin letters, digits and punctuation; anything from another script means
 * the generation is damaged and should not be kept.
 */
const FOREIGN = /[Ͱ-᳿⺀-﫿︰-﹏＀-￯]/;

function assertClean(text) {
  const m = String(text || "").match(FOREIGN);
  if (m) {
    const at = text.indexOf(m[0]);
    const e = new Error("text corupt de la model: „" + text.slice(Math.max(0, at - 20), at + 20) + "”");
    e.corrupt = true;
    throw e;
  }
  return text;
...
module.exports = { ask, askBalanced, available, PROVIDERS, budgetStatus, assertBudget, chargeBudget, assertClean, DAILY_CALLS };
