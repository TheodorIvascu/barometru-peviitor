...
async function build({ minJobs = 100 } = {}) {
  ...
  for (const [host, jobs] of total) {
    ...
    // the full source-by-rule grid the Surse tab draws, as a share of the
    // source's own jobs and as raw counts; `top` stays because the cards and
    // the model prompt only ever want four
    const cells = {};
    const counts = {};
    for (const [id, n] of Object.entries(e.rules)) {
      cells[id] = +((n / jobs) * 100).toFixed(1);
      counts[id] = n;
    }
