...
const RULES = [
  // ---- locations ---------------------------------------------------------
  {
    ...
    hint: "Valori ca „all”, „Nespecificat”, „Toate” sau coduri interne. Nu spun unde e jobul.",
  ...
  },
  {
    ...
    hint: "„Romania” sau „Remote, Romania”. Corect pentru remote, dar fără localitate.",
  ...
  },
  {
    ...
    hint: "Joburi reale, în străinătate. De etichetat, nu de șters.",
  ...
  // ---- companies / CIF ---------------------------------------------------
  {
    ...
    hint: "CIF-ul jobului nu există în core-ul `company`. Zerourile din față sunt ignorate la comparație.",
  ...
  },
  {
    id: "cif_bad_checksum",
    label: "CIF invalid",
    hint: "Nu trece cifra de control oficială, deci nu poate fi un CIF real.",
    group: "Companii", severity: "blocant", weight: 10,
    tier: "scan", needs: "url,title,company,cif,location",
  ...
  },
  {
    ...
    hint: "„IONUT&amp;MADA S.R.L.”: entități HTML rămase de la scraper.",
  ...
  // ---- titles ------------------------------------------------------------
  {
    ...
    hint: "Videochat, escortă, masaj erotic. „Animatoare” apare și în joburi obișnuite, deci lista cere verificare.",
  ...
  },
  {
    ...
    hint: "Număr de telefon sau adresă de email rămase în titlu.",
  ...
  },
  {
    ...
    hint: "Același titlu la aceeași companie, apărut de mai multe ori.",
  ...
  // ---- completeness ------------------------------------------------------
  {
    ...
    hint: "Câmpul job_type e gol: nu se știe dacă e la birou, hibrid sau remote.",
    group: "Câmpuri goale", severity: "info", weight: 4,
    tier: "solr", q: "-workmode:*",
  },
  {
    id: "missing_tags",
    label: "Fără etichete",
    hint: "Câmpul hashtags e gol, deci jobul nu apare în filtrele pe domeniu.",
    group: "Câmpuri goale", severity: "info", weight: 4,
    tier: "solr", q: "-tags:*",
  },
  ...
  // Fiecare regulă de aici citează o cerință explicită din Job Model Schema.
  {
    ...
    hint: "Schema cere denumirea legală, scrisă cu majuscule.",
    group: "Abateri de la schemă", severity: "avertisment", weight: 5,
    tier: "scan", needs: "url,title,company,cif,location",
    test: (d) => {
  ...
  },
  {
    ...
    hint: "Schema cere exact 8 cifre, fără prefixul RO. Așa se leagă de core-ul `company`.",
    group: "Abateri de la schemă", severity: "avertisment", weight: 6,
    tier: "scan", needs: "url,title,company,cif,location",
    test: (d) => {
  ...
  },
  {
    ...
    hint: "Schema cere etichete fără diacritice, ca să se potrivească între surse.",
    group: "Abateri de la schemă", severity: "avertisment", weight: 5,
  ...
  },
  {
    ...
    hint: "Schema cere etichete cu litere mici.",
    group: "Abateri de la schemă", severity: "info", weight: 3,
    tier: "scan", needs: "url,title,company,tags",
  ...
  },
  {
    ...
    hint: "Schema limitează la 20 de etichete pe job.",
    group: "Abateri de la schemă", severity: "info", weight: 3,
    tier: "scan", needs: "url,title,company,tags",
  ...
  },
  {
    ...
    hint: "Schema permite doar „remote”, „on-site” sau „hybrid”.",
    group: "Abateri de la schemă", severity: "blocant", weight: 8,
  ...
  },
  {
    ...
    hint: "Schema permite doar: scraped, tested, verified, published.",
    group: "Abateri de la schemă", severity: "blocant", weight: 8,
  ...
  },
  {
    ...
    hint: "Schema limitează titlul la 200 de caractere.",
    group: "Abateri de la schemă", severity: "avertisment", weight: 5,
    tier: "scan", needs: "url,title,company,location",
  ...
  },
  {
    id: "title_html",
    label: "HTML în titlu",
    hint: "Schema cere titlu fără marcaj HTML.",
    group: "Abateri de la schemă", severity: "blocant", weight: 8,
    tier: "scan", needs: "url,title,company,location",
  ...
  },
  {
    ...
    hint: "Schema cere titlul fără spații la început sau la final.",
    group: "Abateri de la schemă", severity: "info", weight: 2,
    tier: "scan", needs: "url,title,company,location",
    test: (d) => {
  ...
  },
  {
    ...
    hint: "Schema cere formatul „MIN-MAX MONEDĂ”, de exemplu „5000-8000 RON”.",
    group: "Abateri de la schemă", severity: "avertisment", weight: 4,
  ...
  },
  {
    ...
    group: "Abateri de la schemă", severity: "blocant", weight: 10,
    tier: "scan", needs: "url,title,company,location",
    test: (d) => {
...
/**
 * Counts for a container whose cache/ has not been built yet.
 *
 * The full scan writes the id of every offending job, which is why it weighs
 * megabytes and does not survive a restart. The snapshot keeps only the counts,
 * so the dashboard opens with real numbers straight away while the fresh run
 * happens behind it. Rows cannot be opened from these - see lib/snapshot.js.
 */
function readSnapshotCounts() {
  const snap = require(path.join(__dirname, "snapshot.js")).read();
  if (!snap || !snap.counts) return null;
  return { at: snap.at, scanned: snap.scanned, counts: snap.counts, stale: true, from: snap.from };
}
...
async function counts() {
  ...
  const cache = readCache() || readSnapshotCounts();
  const out = [];
  ...
  return {
    ...
    stale: !!(cache && cache.stale),        // counts came from the snapshot
    source: cache && cache.from ? cache.from : "cache",
    rules: out,
  };
}
